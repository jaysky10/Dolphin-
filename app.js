...
function editSale(id) {
  ...
  if (newProduct && !isNaN(newQuantity) && !isNaN(newPrice)) {
...
function saveShopee() {
  ...
  tracker.push({ id: Date.now(), trackingNumber, amount, released });
...
function renderShopee() {
  ...
  tracker.forEach(t => {
    list.innerHTML += `
      <div>
        ${t.trackingNumber} - ₱${t.amount} - Released: 
        <input type="checkbox" ${t.released ? 'checked' : ''} onchange="toggleShopee(${t.id}, this.checked)">
...
function drawChart() {
  ...
  shopee.forEach(t => {
    if (t.released) {
      ...
      daily[today] = daily[today] || { sales: 0, expenses: 0 };
      daily[today].sales += t.amount;
    }
  });
